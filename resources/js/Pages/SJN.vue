    ...
    setup(props){
        ...
        const correrAlgoritmo = () =>{
            ...
            if(ejecucion.value == null && listos.value.length > 0){
                ejecucion.value = 
                listos.value.reduce(function(ant, act) {
                    return ant.rafagas < act.rafagas ? ant : act;
                });
                
                rafagas_ejecucion.value = ejecucion.value.rafagas;

                listos.value.splice(listos.value.findIndex((proceso) => {
                    proceso.id === ejecucion.value.id
                }),1);
            }
